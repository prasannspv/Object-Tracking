...
def run_particle_filter(filter_class, imgs_dir, template_rect,
    ...
    pf = None
    frame_num = 0
    bh, bw, _ = cv2.imread(os.path.join(imgs_dir, imgs_list[0])).shape
    out_path = "OUT_VIDEO_1.mp4"
    video_out = mp4_video_writer(out_path, (bw, bh))
    ...
    for img in imgs_list:
        ...
        if True:  # For debugging, it displays every frame
            ...
            pf.render(out_frame)
            out_frame = cv2.putText(out_frame, "Prasanna", (50, 20), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255))
...
def run_particle_filter_multiple(filter_class, imgs_dir, template_rect,
    ...
    for img in imgs_list:
        ...
        # Extract template and initialize (one-time only)
        if len(template) == 0:
            for templ in template_rect:
                template.append(frame[int(templ['y']):
                       int(templ['y'] + templ['h']),
                        int(templ['x']): int(templ['x'] + templ['w'])])

            if 'template' in save_frames:
                cv2.imwrite(save_frames['template'], template)

            pf = []
            for templ, rect, alpha, sigma_exp, sigma_dyn in zip(template, template_rect, kwargs['alpha'], kwargs['sigma_exp'], kwargs['sigma_dyn']):
                kwargs['template_coords'] = rect
                kwargs['alpha'] = alpha
                kwargs['sigma_exp'] = sigma_exp
                kwargs['sigma_dyn'] = sigma_dyn
                # if count == 3:
                #     filter_class = ps5.MDParticleFilter
                #     kwargs['scale'] = 1
                #     kwargs['update_scale'] = 1
                #     kwargs['mse'] = 1700
                # count += 1
                pf.append(filter_class(frame, templ, **kwargs))

        count = 1
        # Process frame
        for pf_run in pf:
            if (count == 1 and frame_num >= 27) or (count == 2 and frame_num <= 58) or count == 3:
                pf_run.process(frame)
            count += 1
        ...
        if True:  # For debugging, it displays every frame
            ...
            count = 1
            for pf_run in pf:
                if (count == 1 and frame_num >= 27) or (count == 2 and frame_num <= 58) or (count == 3 and frame_num <= 46):
                    pf_run.render(out_frame)
                count += 1
            cv2.imshow('Tracking', out_frame)
            cv2.waitKey(100)
...
def part_1b():
    ...
    kf = ps5.KalmanFilter(template_loc['x'], template_loc['y'], Q, R)
...
def part_1c():
    ...
    kf = ps5.KalmanFilter(init_pos['x'], init_pos['y'], Q, R)
...
def part_5():
    ...
    """
    # template_rect = [{'x': 83, 'y': 154, 'w': 64, 'h': 282},
    #                  {'x': 294, 'y': 200, 'w': 47, 'h': 172},
    #                  {'x': 0, 'y': 175, 'w': 49, 'h': 237}]
    template_rect = [{'x': 15, 'y': 175, 'w': 50, 'h': 200}, {'x': 83, 'y': 154, 'w': 64, 'h': 282},{'x': 300, 'y': 220, 'w': 40, 'h': 100}]
    ...
    run_particle_filter_multiple(ps5.AppearanceModelPF,
                                 os.path.join(input_dir, "TUD-Campus"),
                                 template_rect,
                                 save_frames,
                                 num_particles = num_particles, sigma_exp = [5, 10, 5],
                                 sigma_dyn = [5, 10, 30],
                                 template_coords = template_rect, alpha=[0.6, 0.3, 0.02])  # Add more if you need to
...
def part_6():
    ...
    Place all your work in this file and this section.
    """
    template_rect = {'x': 89, 'y': 33, 'w': 40, 'h': 90}
